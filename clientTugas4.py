...
def deliver_message():
    data = request.json
    sender = data["from"]
    cipher_message = data["cipher"]
    cipher_des_key = int(data["cipher_des_key"])
    ...
    des_key_bytes = int_to_bytes(rsa_decrypt(cipher_des_key, RSA_PRIV))
    des_key_hex = ''.join(f"{b:02X}" for b in des_key_bytes)[:16]
    subkeys = generate_subkeys_from_keyhex(des_key_hex)

    plain_blocks = []
    for i in range(0, len(cipher_message), 16):
        block = cipher_message[i:i+16]
    ...
    valid = False
    if pubkey_sender:
        valid = verify_signature(decrypted, signature, pubkey_sender)

    print(f"\n[CLIENT-{client_id}] Pesan dari {sender}:")
    print(f"Cipher: {cipher_message}")
    print(f"DES Key (decrypted): {des_key_hex}")
...
def send_message(to_id, text):
    ...
    des_key_int = int_from_bytes(des_key_bytes)
    cipher_des_key = rsa_encrypt(des_key_int, pubkey_to)
    ...
    payload = {
        ...
        "to": to_id,
        "cipher": cipher_text,
        "cipher_des_key": str(cipher_des_key),
        "signature": signature
    }
    ...
    try:
        ...
        print(f"DES Key (encrypted): {cipher_des_key}")
...
if __name__ == "__main__":
    print("=== Client Hybrid DES+RSA + Signature ===")
